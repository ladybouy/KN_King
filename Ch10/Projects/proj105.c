...
bool royal, straight, flush, aces_low, four, three;
...
void analyze_hand(void)
{
    ...
    for (rank = 8; rank < NUM_RANKS && num_in_rank[rank] == 1; rank++)
        royal_sum++; 
    if (royal_sum == NUM_CARDS ) {
        royal = true;
        return;
    }
 ...
 *                based on the valuues of the external     *
...
void print_result(void)
{
    ...
    else if (aces_low)          printf("Aces Low");
